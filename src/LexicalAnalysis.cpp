	...
	while (true)
	{
		...
		if (nextState > IDLE_STATE)
		{
			...
			if (nextState == START_STATE)
				throw runtime_error("\n[PROGRAM] : Exception! Infinite state detected! There is something very wrong with the code !\n");
...
void LexicalAnalysis::printTokens()
{
	if (tokenList.empty())
	{
		cout << "[PROGRAM] : Token list is empty!" << endl;
	}
	else
	{
		printMessageHeader();
...
void LexicalAnalysis::printLexError()
{
	...
	{
		cout << "[PROGRAM] : There are no lexical errors!" << endl;
	}
}
